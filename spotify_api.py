...
class SpotifyAPIManager:
    ...
    def get_resource(
        self, endpoint: str, params: list = None, fields: list = None
    ) -> dict:
...
class SpotifyAPI:
    ...
    def playlist_items_batch(
        self, playlist_id: str, offset: int = None, fields: list = None
    ) -> dict:
    ...
    def playlist_items(
        self, playlist_id: str, fields: list = None, playlist_length: int = None
    ) -> dict:
        """Returns the items of the playlist with the given id"""
        if playlist_length is None:
            playlist_length = self.playlist(playlist_id, fields=["tracks.total"])[
                "tracks"
            ]["total"]
        items = []
        for offset in range(0, playlist_length, 100):
            items.extend(
                self.playlist_items_batch(playlist_id, offset, fields)["items"]
            )
        return items
    ...
    def playlist_remove_all_occurrences_of_items(self, playlist_id: str, items) -> bool:
        ...
        for i in range(0, len(items), 100):
            if not self.playlist_remove_all_occurrences_of_items_batch(
                playlist_id, items[i: i + 100]
            ):
                return False
        return True

    def playlist_remove_all_occurrences_of_items_batch(
    ...
    def playlist_add_items(
        self, playlist_id: str, track_ids: list, position: int = None
    ):
        """Adds the given items to the playlist in order"""
        for i in range(0, len(track_ids), 100):
            if not self.playlist_add_items_batch(
                playlist_id, track_ids[i: i + 100], position
            ):
                return False
        return True

    def playlist_add_items_batch(
        self, playlist_id: str, track_ids: list, position: int = None
    ):
        """Adds the given items to the playlist in order"""
        endpoint = f"{self.api_manager.api_url}playlists/{playlist_id}/tracks"
